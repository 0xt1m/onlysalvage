...
import { ImageOff, X, CircleCheckBig } from 'lucide-react'
import { Button } from '@/components/ui/Button'
import { Input } from '@/components/ui/Input'
import { requestDamagePhotos } from '@/lib/api'
import { formatPhoneDigits, formatPhoneNumber, isPhoneNumberComplete, phoneDigitsOnly } from '@/lib/utils'

interface RequestDamagePhotosModalProps {
  slug: string
}

const EMAIL_RE = /^[^\s@]+@[^\s@]+\.[^\s@]+$/

export function RequestDamagePhotosModal({ slug }: RequestDamagePhotosModalProps) {
  const t = useTranslations('RequestDamagePhotos')
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setSubmitting(true)
    const { ok } = await requestDamagePhotos(slug, {
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4">
          <div className="bg-surface border border-border rounded-lg w-full max-w-md max-h-[90vh] overflow-y-auto p-6 flex flex-col gap-4">
            <div className="flex items-center justify-between">
              <h3 className="text-lg font-semibold">{t('requestDamagePhotos')}</h3>
