...
import { LISTING_REPORT_REASONS } from '@/lib/types'
...
// The actual dialog, controlled from outside -- both the "Report" button on
// the listing detail page (ReportListingModal below) and a listing card's
// right-click menu open the same dialog, just from different triggers.
export function ReportListingDialog({ slug, onClose }: ReportListingDialogProps) {
  const t = useTranslations('ReportListingModal')
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setSubmitting(true)
    const { ok } = await reportListing(slug, reason, details.trim())
    setSubmitting(false)
        ...
        {submitted ? (
          ...
          <form onSubmit={handleSubmit} className="flex flex-col gap-4">
            <Select
              ...
              options={translateOptions(LISTING_REPORT_REASONS, (code) => tReason(code))}
...
export function ReportListingModal({ slug }: ReportListingModalProps) {
  const t = useTranslations('ReportListingModal')
  ...
  return (
    <>
      ...
      {open && <ReportListingDialog slug={slug} onClose={() => setOpen(false)} />}
    </>
  )
}
